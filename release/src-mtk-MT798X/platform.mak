...
export DTB_PATH := arch/arm64/boot/dts/mediatek
export WTOOL_SUFFIX=_mt798x
else
export LOADADDR := 40008000
export M32_SUFFIX := "_32bit"
export PLATFORM := arm-musl
export TOOLS := /opt/openwrt-gcc840_musl.arm
...
export DTB_PATH := arch/arm/boot/dts
export WTOOL_SUFFIX=
endif
...
export SDKCONFIG=$(PLATFORM_ROUTER)/sdk.config-7981$(M32_SUFFIX)
endif
...
define PreConfigChange
	@( \
		if [ "$(MT7981)" = "y" ]; then \
			cp -f $(LINUXDIR)/config_base_7981$(M32_SUFFIX) $(LINUXDIR)/config_base; \
		else \
			cp -f $(LINUXDIR)/config_base_7986$(M32_SUFFIX) $(LINUXDIR)/config_base; \
		fi; \
	)
endef
...
define platformKernelConfig
	...
	if [ -n "$(findstring ntfs3, $(NTFS))" ] ; then \
		sed -i "/CONFIG_NTFS3_FS/d" $(1); \
		echo "CONFIG_NTFS3_FS=y" >>$(1); \
		sed -i "/CONFIG_NTFS3_64BIT_CLUSTER/d" $(1); \
		echo "# CONFIG_NTFS3_64BIT_CLUSTER is not set" >>$(1); \
		sed -i "/CONFIG_NTFS3_LZX_XPRESS/d" $(1); \
		echo "CONFIG_NTFS3_LZX_XPRESS=y" >>$(1); \
		sed -i "/CONFIG_NTFS3_FS_POSIX_ACL/d" $(1); \
		echo "# CONFIG_NTFS3_FS_POSIX_ACL is not set" >>$(1); \
	fi ; \
	if [ "$(JFFS2)" = "y" ]; then \
		if [ "$(CONFIG_LINUX30)" = "y" ]; then \
	...
	if [ "$(WIREGUARD)" = "y" ]; then \
		if [ "$(MUSL32)" = "y" ]; then \
			echo "CONFIG_CRYPTO_SHA256_ARM=y" >>$(1); \
			echo "CONFIG_CRYPTO_SHA512_ARM=y" >>$(1); \
			echo "CONFIG_CRYPTO_SHA1_ARM_CE=y" >>$(1); \
			echo "CONFIG_CRYPTO_SHA2_ARM_CE=y" >>$(1); \
			echo "CONFIG_CRYPTO_SHA512_ARM_CE=y" >>$(1); \
			echo "CONFIG_CRYPTO_SHA3_ARM=y" >>$(1); \
			echo "CONFIG_CRYPTO_SM3_ARM_CE=y" >>$(1); \
			echo "CONFIG_CRYPTO_SM4_ARM_CE=y" >>$(1); \
			echo "CONFIG_CRYPTO_GHASH_ARM_CE=y" >>$(1); \
			echo "CONFIG_CRYPTO_AES_ARM=y" >>$(1); \
			echo "CONFIG_CRYPTO_AES_ARM_CE=y" >>$(1); \
			echo "CONFIG_CRYPTO_AES_ARM_CE_CCM=y" >>$(1); \
			echo "CONFIG_CRYPTO_AES_ARM_CE_BLK=y" >>$(1); \
			echo "CONFIG_CRYPTO_AES_ARM_NEON_BLK=y" >>$(1); \
			echo "CONFIG_CRYPTO_CHACHA20_NEON=y" >>$(1); \
			echo "CONFIG_CRYPTO_POLY1305_NEON=y" >>$(1); \
			echo "CONFIG_CRYPTO_NHPOLY1305_NEON=y" >>$(1); \
			echo "CONFIG_CRYPTO_AES_ARM_BS=y" >>$(1); \
		else \
