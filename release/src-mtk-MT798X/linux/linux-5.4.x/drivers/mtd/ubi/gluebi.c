...
void glue_isp_create(char *parasite_name, unsigned int r_len, struct mtd_partition *mp)
{
	struct gluebi_device *gluebi, *g;
	list_for_each_entry_safe(gluebi, g, &gluebi_devices, list) {
		struct mtd_info *mtd = &gluebi->mtd;
		if (mtd->name && !strcmp(mtd->name, parasite_name)) {
			int ret;
			size_t rdlen;
			unsigned char tmpbuf[4];
			ret = gluebi_read(mtd, mtd->size - r_len, sizeof(tmpbuf), &rdlen, tmpbuf);
			if (ret || rdlen != sizeof(tmpbuf)) {
				pr_err("%s: read %lx data from 0x%llx of mtd %s fail. (ret = %d, rdlen %zx)\n",
					__func__, sizeof(tmpbuf), mtd->size - r_len, mtd->name, ret, rdlen);
			} else {
				//pr_crit("VVVVVVV %02x:%02x:%02x:%02x\n", tmpbuf[0], tmpbuf[1], tmpbuf[2], tmpbuf[3]);
				if (*((u32 *)tmpbuf) == CRAMFS_MAGIC) {
					mp->offset = mtd->size - r_len;
					mp->size = r_len;
					printk(KERN_INFO "ISP offset %llx, size:%llx\n", mp->offset, mp->size);
					mtd_device_register(mtd, mp, 1);
				}
			}
			break;
		}
