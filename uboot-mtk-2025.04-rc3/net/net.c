...
int net_loop(enum proto_t protocol)
{
	...
	int i=0;
#endif
#if defined(CONFIG_ASUS_PRODUCT) && (defined(RESCUE_SWITCH2_RESET_DEFAULT) || defined(RST_BTN_WPS_HARDRESET))
	ulong time_checked;
#endif
...
restart:
	...
	case 0:
		...
		case TFTPD:
			printf("tftpd start\n");
			TftpdStart();
#if defined (CONFIG_ASUS_WEBUI)
			{
				int prepare_asus_httpd(void);
				prepare_asus_httpd();
			}
			mtk_tcp_start();
	...
	for (;;) {
		...
		if ((protocol == MTK_TCP) ||
#if defined(CONFIG_ASUS_WEBUI)
			(protocol == TFTPD && !g_rescue_data_going)
#else
			0
#endif
		   )
...
#elif defined(CONFIG_ASUS_PRODUCT) && defined(RST_BTN_WPS_HARDRESET)
		if (g_rescue_mode && !g_rescue_data_going && ((get_timer(0) - time_checked) > (CONFIG_SYS_HZ/20))) {
