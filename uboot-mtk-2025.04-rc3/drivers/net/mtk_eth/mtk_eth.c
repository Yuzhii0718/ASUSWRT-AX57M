...
static struct mtk_eth_priv *bk_priv = NULL;
int mii_c45_r(u8 addr, u8 devad, u16 reg)
{
	if (bk_priv)
		return mtk_mmd_read(bk_priv, addr, devad, reg);
	return 0;
}

int mii_c45_w(u8 addr, u8 devad, u16 reg, u16 val)
{
	if (bk_priv)
		return mtk_mmd_write(bk_priv, addr, devad, reg, val);
	return 0;
}
...
static int mtk_eth_of_to_plat(struct udevice *dev)
{
	...
	if (priv->swname) {
		...
		if (priv->phy_interface == PHY_INTERFACE_MODE_10GBASER) {
#if defined(GS7)
			// reset MXL86282
			uint32_t wdt_sta = *(volatile uint32_t*)(0x1001C000 + 0x0c);
			if (wdt_sta) { // not cold boot
				printf("WDT_STA(%x), reset MXL862xx!\n", wdt_sta);
				gpio_request(3, "boot");
				gpio_direction_output(3, 0); // assert
				mdelay(300);
				gpio_direction_output(3, 1); // de-assert
			}
#endif
			return 0;
		}
		...
		if (priv->gmac_id == 0) {
			...
			if (wdt_sta) { // not cold boot
				printf("WDT_STA(%x), reset MXL862xx!\n", wdt_sta);
				gpio_request(42, "boot");
				gpio_direction_output(42, 0); // assert
				mdelay(300);
				gpio_direction_output(42, 1); // de-assert
			}
			//printf("skip gmaic_id 0\n");
			return 0;
		}
