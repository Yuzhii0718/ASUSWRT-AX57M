...
#if defined(CONFIG_ASUS_PRODUCT)
#if 0
static int ico_val(void)
{
	int i;
	char *s, *end, *def_mac_str = MK_STR(CONFIG_ETHADDR);
	uchar *m, mac[6] = { 0 }, def_mac[6] = { 0 };

	if ((s = env_get("rdm")) && simple_strtoul(s, NULL, 0) == 0xc0)
		return 0;

	if (ra_factory_read(mac, CFG_MAC_OFFSET, sizeof(mac)))
		return 0;

	for (i = 0, s = def_mac_str, m = def_mac; i < 6; ++i, ++m) {
		*m = s? simple_strtoul(s, &end, 16):0;
		if (s)
			s = (*end)? end + 1 : end;
	}
	if (!memcmp(mac, def_mac, sizeof(mac)))
		return 0;
...
#endif
#if defined(CONFIG_ASUS_PRODUCT)
#ifdef CONFIG_FDT_NO_BOOTARGS_OVERRIDE
	char  *str;		// we need it
#endif
...
#endif
			if (i != -1) {
#if defined(CONFIG_ASUS_PRODUCT) && !defined(CONFIG_TARGET_MT7988) // 2G/LAN/WAN use the same MAC
				strcpy(mac, "ethaddr");
#else
				// MT7988, LAN(eth0), LAN2(eth2) set to 2G MAC, WAN(eth1) set to 5G MAC
				if (i == 0 || i == 2)
					strcpy(mac, "ethaddr");
				else
