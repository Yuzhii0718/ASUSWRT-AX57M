...
int env_loc = 0;
int ubi_damaged = 0;
#ifdef CONFIG_ASUS_WEBUI
int using_httpd = 0;
int httpd_reset = 0;
#endif	/* CONFIG_ASUS_WEBUI */
...
extern struct bootm_headers images;		/* pointers to os/initrd/fdt images */
...
void switch_r2default_detect()
{
	...
	if ( gl_switch_val_xcount >= 2 ) {
		...
		reset_to_default();
		while ( i++ < 3) {
#if defined(PRTAX57_GO) || defined(EBA76)
			asus_blue_led_on();
#else
			power_led_on();
#endif
...
static int bootm_warpper(char *argv[], int extra_flags, int boot)
{
	...
	bmi.argv = argv;
	if (!boot)
		return bootm_run_states(&bmi, BOOTM_STATE_START | BOOTM_STATE_FINDOS | extra_flags);
	else
		return bootm_run_states(&bmi, BOOTM_STATE_LOADOS | BOOTM_STATE_OS_PREP | BOOTM_STATE_OS_GO | extra_flags);
}
...
{
#ifdef CONFIG_ASUS_WEBUI
	if(using_httpd > 1 && images.os.image_len > 0)
		return sizeof(struct legacy_img_hdr) + images.os.image_len;	// firmware had checked and okay
#endif	/*/ CONFIG_ASUS_WEBUI */

	images.verify = 1;
	if (!bootm_warpper(NULL, 0, 0))
		return sizeof(struct legacy_img_hdr) + images.os.image_len;

	return -1;
}

#ifdef RST_BTN_WPS_HARDRESET
//#define VV_DEBUG
#define	RST_HARD_RESET_SECONDS	5
#define	CONFIG_HZ_MULTIFIER	1
void rst_hardreset_detect(int state)
{
	static ulong start_time;
	static int give_up;
	ulong cur_time;
#ifdef VV_DEBUG
	static ulong pr_time;
#endif

	if (state == 0) {
		if (!DETECT()) {	/* Reset to default button, released */
			give_up = 1;
#ifdef VV_DEBUG
			printf("[%s:%d], RST released!\n", __func__, __LINE__);
#endif
		}
		else {
...
#ifdef VV_DEBUG
			pr_time = start_time;
			printf("[%s:%d], START time:%lu\n", __func__, __LINE__, pr_time);
#endif
			give_up = 0;
		}
		return;
	...
	if (give_up) // already give up
		return;

	cur_time = get_timer(0);
#ifdef VV_DEBUG
	if ((cur_time - pr_time) >= (CONFIG_SYS_HZ/2)) {
		printf("[%s:%d], cur_time:%lu\n", __func__, __LINE__, cur_time);
		pr_time = cur_time;
	}
#endif

	if ((cur_time - start_time) >= (CONFIG_SYS_HZ * RST_HARD_RESET_SECONDS * CONFIG_HZ_MULTIFIER)) {
                //int i = 0;
                printf("\n\n!!!! RST HARD reset default !!!!\n\n");
                reset_to_default();
                while ( DETECT() /*i++ < 3*/) {
                        asus_blue_led_on();
                        mdelay(100);
...
static int get_firmware_image(ulong img_addr, ulong ram_addr, ulong *total_len_pt)
{
	...
	*total_len_pt = 0;
	...
	case IMAGE_FORMAT_LEGACY:
		d_size = image_get_data_size((struct legacy_img_hdr *)ram_addr);
...
static int load_asus_firmware(void)
{
	...
	if (ret == 0) {
		...
		ret = bootm_warpper(argv, BOOTM_STATE_FINDOTHER, 0);
#else
...
#if defined(RTAX59U) || defined(PRTAX57_GO) || defined(BT8) || defined(BT6) || defined(BT8P) || defined(GS7) || defined(GSBE7200X) || defined(EBA76) || defined(GS7_MAGIC)
		asus_purple_led_on();
#endif
		wan_red_led_on();
#if 0 //defined(CONFIG_PWM_MTK_MM)
		bbtype(2);
#endif
#ifdef RESCUE_SWITCH2_RESET_DEFAULT
...
#if defined(RTAX59U)
					asus_red_led_on();
#elif defined(BT8)|| defined(BT6) || defined(BT8P) || defined(EBA76)
					asus_blue_led_on();
#else
#if defined(GS7) || defined(GSBE7200X) || defined(GS7_MAGIC)
					asus_blue_led_on();
#endif
					power_led_on();
#endif
...
static void RAMtoFlash(void)
{
	...
	else
	{
		...
		{
#ifdef CONFIG_ASUS_WEBUI
			//fail and recover httpd
			printf("last   net_ip(%pI4) net_gateway(%pI4) net_netmask(%pI4) net_server_ip(%pI4)\n", &net_ip, &net_gateway, &net_netmask, &net_server_ip);
			net_ip = env_get_ip("ipaddr");
			net_gateway = env_get_ip("gatewayip");
			net_netmask = env_get_ip("netmask");
			net_server_ip = env_get_ip("serverip");
			printf("set to net_ip(%pI4) net_gateway(%pI4) net_netmask(%pI4) net_server_ip(%pI4)\n", &net_ip, &net_gateway, &net_netmask, &net_server_ip);
#endif	/* CONFIG_ASUS_WEBUI */
		...
		/* skip .trx header */
		hdr = (void*) (ptr + sizeof(struct legacy_img_hdr));
		copysize -= sizeof(struct legacy_img_hdr);
...
static void SolveTRX(void)
{
	...
	if (check_trx() <= 0) {
		...
		rc = ra_flash_erase_write(ptr+sizeof(struct legacy_img_hdr), CFG_KERN_ADDR, count-sizeof(struct legacy_img_hdr), 0);
#if defined(CFG_DUAL_TRX)
		if (rc)
			printf("Write 1st firmware fail. (r = %d)\n", rc);
		if (mtd->size > 0x8000000) { // have linux2
			rc = ra_flash_erase_write(ptr+sizeof(struct legacy_img_hdr), CFG_KERN2_ADDR, count-sizeof(struct legacy_img_hdr), 0);
			if (rc)
...
{
	printf("%s: ptr(%p) copysize(%u) data(%p) size(%zu) image_load_addr(%lu)\n", __func__, ptr, copysize, data, size, image_load_addr);

	using_httpd = 1;
	ptr = (unsigned char *)data;
	image_load_addr = (ulong)data;
	copysize = size;

	if (!memcmp(ptr, trx_magic, sizeof(trx_magic)) && (copysize > CONFIG_MAX_TRX_IMAGE_SIZE)) {
		int fw_size;
		fw_size = check_trx();
		printf("%s() fw_size(%d) copysize(%u)\n", __func__, fw_size, copysize);
		if (fw_size > 0 && copysize > 0) {
			return 0;
		}
	}
	else {
		...
		if (copysize > 0 && copysize <= CONFIG_MAX_TRX_IMAGE_SIZE)		// uboot
		{
			...
			printf("Not trx and not uboot file\n");
	}
	printf("%s fail\n", __func__);
	/* recover TFTPD */
	net_set_state(STATE_BAD_MAGIC);
	...
	TftpBlock = 0;
	RescueAckFlag= 0x0000;
	using_httpd = 0;	//finish httpd protocol
	copysize = 0;
	return -1;
}
...
int failsafe_write_image(const void *data, size_t size)
{
	...
	copysize = size;
	rc = 0;
	RAMtoFlash();	// write to flash and return global rc value
	if(rc) {	//fail
		/* recover TFTPD */
		net_set_state(NETLOOP_FAIL);
		TftpTimeoutCount = 0;
		...
		//net_set_state(NETLOOP_SUCCESS);	//set to NETLOOP_SUCCESS would reboot automatically but no httpd result is shown in web page.
	}
	return rc;
}
#endif /* CONFIG_ASUS_WEBUI */
