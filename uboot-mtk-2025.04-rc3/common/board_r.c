...
static void str2mac(unsigned char *mac, char *default_ethaddr_str)
{
	int i;
	char *tmp, *end;
...
void init_mac(void)
{
	...
	printf("\ninit MAC\n");
	mac_str = MK_STR(CFG_ETHADDR);
	str2mac(mac, mac_str);
	__call_replace(CFG_EEPROM_OFFSET + CFG_MAC_OFFSET, mac, sizeof(mac), "init 2G mac");
#if defined(CONFIG_TARGET_MT7988)
	mac_str = MK_STR(CFG_ETHADDR_5G);
	str2mac(mac, mac_str);
	__call_replace(CFG_EEPROM_OFFSET + CFG_MAC_OFFSET_5G, mac, sizeof(mac), "init 5G mac");

	mac_str = MK_STR(CFG_ETHADDR_6G);
	str2mac(mac, mac_str);
	__call_replace(CFG_EEPROM_OFFSET + CFG_MAC_OFFSET_6G, mac, sizeof(mac), "init 6G mac");
#endif
...
static void handle_boottype_2(void)
{
	...
	{
		ra_flash_erase_write((uchar*)load_address+sizeof(struct legacy_img_hdr), CFG_KERN_ADDR, net_boot_file_size-sizeof(struct legacy_img_hdr), 0);
#if defined(CFG_DUAL_TRX)
		if (mtd->size > 0x8000000) // have linux2
		ra_flash_erase_write((uchar*)load_address+sizeof(struct legacy_img_hdr), CFG_KERN2_ADDR, net_boot_file_size-sizeof(struct legacy_img_hdr), 0);
#endif
	}
	getHWID();
#if defined(CONFIG_PWM_MTK_MM)
	...
	argc= 2;
	sprintf(addr_str, "0x%lX#config-1", load_address+sizeof(struct legacy_img_hdr));
	argv[1] = &addr_str[0];
...
static int run_main_loop(void)
{
	...
	asus_set_ethmac(); // must be called after chkMAC

#if defined(GS7_MAGIC)
	bbtype(3);
#endif
menu_select:
